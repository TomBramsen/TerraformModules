...
resource "azurerm_kubernetes_cluster" "k8s" {
  ...
  default_node_pool {
    name                   = "agentpool"
    vm_size                = "Standard_DS2_v2" # "Standard_B2als_v2" #
    ...
    zones                  = [1,2,3] # try([for zone in local.regions_by_name_or_display_name[var.location].zones : zone], null)
...
/*
  linux_profile {
    admin_username = "sa" # var.username
...
resource "azurerm_kubernetes_cluster_node_pool" "this" {
  ...
  tags                  = var.tags
  vnet_subnet_id        = var.subnet_id # module.avm_res_network_virtualnetwork.subnets["subnet"].resource_id
